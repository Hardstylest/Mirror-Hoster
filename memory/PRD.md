...
- **Thumbnail-Befund (NICHT gelöst)**: Provider-Cover sind serverseitig NICHT abrufbar: DoodStream-Bilddomains tot (`doimg.net` DNS-tot, `dodoimg.com` HTTP 451 CF-Block); VOE-Embed 403; Firestream `posterUrl` → 404 (session-/hotlink-geschützt). Automatisches Ziehen (User-Wunsch Option b) daher nicht zuverlässig umsetzbar. Empfehlung an User: manuelles Cover-Feld pro Mirror (Option a). OFFEN – wartet auf User-Entscheidung.
