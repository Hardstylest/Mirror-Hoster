...
## Statistik: Verdienst pro Land + zeitraumabhängige Top-Hoster (2026-06)
- **Verdienst pro Land** (`server.py` stats_overview): in der mc_agg-Schleife wird der geschätzte Verdienst zusätzlich pro `country_code` akkumuliert (`country_earn`), Response-Feld `earnings_by_country` (Top 12, desc). UI: neues Panel `panel-earn-country` mit Flagge + Klarname + `~$`-Betrag (Balken skaliert auf max), i18n `statsov.earnByCountry`.
- **Hoster-Views mit Zeitstempel** (`server.py` record_host_view): pro Abruf wird jetzt zusätzlich ein Event in neue Collection `host_view_events` {id, slug, host_id, timestamp} geschrieben (Legacy-Counter `host_views` bleibt für Rückwärtskompatibilität / mirror_stats erhalten).
- **Top-Hoster zeitraumabhängig**: bei period != all aggregiert stats_overview `host_view_events` gefiltert nach `timestamp>=start` (+ slug-Scope für Nicht-Admin); bei period == all weiterhin der komplette Legacy-Counter (vollständige Historie). i18n-Hinweis `statsov.hostsNote` angepasst ("Zeitraum-Zählung ab Aktivierung; 'Gesamt' zeigt die Gesamtsumme.").
- Verifiziert: curl — earnings_by_country liefert US/NL/DE; nach 1 aufgezeichnetem host-view zeigt 7d/today top_hosts=1 während all die Legacy-Totals (38/30/8) zeigt; Frontend-Screenshot beide Panels rendern. ERFORDERT REDEPLOY für Production.
- HINWEIS: Zeitraum-gefilterte Hoster-Views existieren erst ab jetzt (keine historischen Events). Legacy-Host-IDs ohne aktiven Hoster erscheinen im all-Top-Hoster mit roher ID (kosmetisch, Altdaten).

## Statistik: Hoster nach Land + Verdienst-Zeitkurve (2026-06)
- **Verdienst-Zeitkurve** (`server.py` stats_overview + `Statistics.jsx`): `timeline` liefert jetzt zusätzlich `earnings` pro Tag. Neue Aggregation `dmc_agg` gruppiert Views nach {Tag, Mirror, Land}; Verdienst je Tag = Σ n*best_rate(mirror,land)/1000 (Helper `best_rate` mit Cache, in mc-Loop + Timeline wiederverwendet). Chart: zweite grüne Linie (`#22C55E`) auf rechter Y-Achse ($-Ticks) + Recharts `Legend`. i18n `statsov.viewsLine/earnLine`.
- **Hoster nach Land** (`record_host_view` + stats_overview): `record_host_view` erkennt jetzt das Land (cf-ipcountry, sonst geolocate(get_geo_ip)) und speichert `country_code` im `host_view_events`-Doc. stats_overview aggregiert `host_view_events` nach {country, host_id} (period-scoped via timestamp, slug-scope für Nicht-Admin) → Response `hosts_by_country` (pro Land: total + Top-4-Hoster). UI-Panel `panel-host-country`: je Land Flagge+Name+Gesamt, darunter Balken je Hoster (Top-Hoster in Markenfarbe hervorgehoben). i18n `statsov.hostByCountry/hostByCountryNote`.
- Verifiziert: curl (timeline mit earnings; hosts_by_country liefert US/XX mit DoodStream) + Frontend-Screenshot (Doppellinien-Chart mit Legende, Hoster-je-Land-Panel). ERFORDERT REDEPLOY für Production.
- HINWEIS: Im Emergent-Preview kommt kein cf-ipcountry an → Land fällt auf Geo-Lookup der LB-IP (US) bzw. XX zurück. In Production (echte Cloudflare-Header) korrekt. hosts_by_country basiert auf host_view_events (erst ab Einführung, keine historischen Länder).

## Statistik-Fix: Top-Hoster Duplikate zusammenfassen + UUID-Müll entfernen (2026-06)
- **Problem** (Prod-Screenshot): Top-Hoster listete mehrere DoodStream-Varianten getrennt (doodstream.com / dooodster.com / myvidplay.com) und zeigte rohe host_id-UUIDs von gelöschten Hostern.
...
## JuicyAds-Verifizierung + Admin Head/Footer-Code + Offline-Edit-Overlay (2026-06)
- **JuicyAads-Meta statisch**: `<meta name="juicyads-site-verification" content="c7e19fdc994ae841f180dc6999eef0c2">` fest in `frontend/public/index.html` <head> eingefügt (zuverlässig für Verifizierungs-Crawler, da SPA-Dokument für alle Routen).
- **Admin Head/Footer-Code**: Neue Settings-Felder `custom_head` + `custom_footer` (backend: SettingsInput, DEFAULT_SETTINGS, PUBLIC_SETTINGS_KEYS — öffentlich lesbar). Admin-UI im Tab "Seiten-Einstellungen" (AdminDashboard.jsx): zwei Textareas `setting-custom-head`/`setting-custom-footer` mit Hinweistext, i18n `admin.settings.customHead*`/`customFooter*`. Runtime-Injektion in `SettingsContext.jsx` via `injectCustomHTML(html, target, marker)`: parst HTML, erzeugt <script> neu (damit sie ausführen), entfernt vorherige Injektionen (`data-gp-inject`) vor Neu-Anwendung. Head→document.head, Footer→document.body. Greift auf ALLEN Seiten (auch /embed, /watch).
- **Offline-Edit-Overlay**: Edit-Button in `OfflineStreams.jsx` öffnet jetzt dasselbe MirrorEditor-Overlay (State `editFor`, Modal `offline-edit-modal`), statt auf `/dashboard/edit/:id` zu navigieren. Nach Save schließt + `load()`. Konsistent mit Dashboard- und /watch-Editor.
- Verifiziert: Backend-Roundtrip (PUT/GET admin+public settings), Screenshots (Admin-Felder sichtbar; Offline-Edit-Overlay vorausgefüllt mit offline-VOE rot markiert). Preview-DB custom_head/footer nach Test auf "" zurückgesetzt.
- ERFORDERT REDEPLOY für Production.

## Code-Check + Verifizierungs-Tab + Pre-Roll-Erweiterungen (2026-06)
- **Code-Warnung (Task 1)**: Helper `codeIssues(code, de)` + `<CodeWarn>` in AdminDashboard.jsx prüft eingegebenen Roh-Code auf: ungleiche <script>/</script>, <style>/</style>, ungleiche </> Zeichen, endet mit offenem Tag. Amber-Warnung (testid `code-warning`) unter Feldern: custom_head, custom_footer, ad_preroll, ad_postroll.
- **Verifizierungs-Tab (Task 2)**: Neuer Admin-Tab `verify` (`admin-tab-verify`, Panel `verify-panel`) mit Feldern verify_juicyads / verify_google / verify_bing. Backend-Settings-Felder + PUBLIC_SETTINGS_KEYS. SettingsContext baut daraus Meta-Tags (google-site-verification, msvalidate.01, juicyads-site-verification) und injiziert sie in <head> (Marker "verify", akzeptiert Token ODER komplettes Meta-Tag via content="..."-Extraktion). Statischer JuicyAds-Meta in index.html bleibt (zuverlässig für JS-lose Crawler); JuicyAds-Feld nur bei Änderung nötig.
- **Pre-Roll-Erweiterungen (Task 3)**: Nutzer meinte Pre-Roll (nicht Overlay). Neue Settings: ad_preroll_repeat_enabled/ad_preroll_repeat_minutes (Werbung während des Schauens in festem Abstand wiederholen) + ad_postroll_enabled/ad_postroll/ad_postroll_minutes (Post-Roll nach fester Schaudauer, da externe Player kein Video-Ende-Signal liefern — im UI klar vermerkt). EmbedPlayer übergibt `ads`-Prop; VideoPlayer.jsx zeigt Overlay (`recurring-ad-overlay`) ÜBER dem laufenden iframe (bg-black, z-20) mit Countdown/Skip (gleiche Logik wie Pre-Roll). Recurring = setInterval, Post-Roll = setTimeout (one-shot); beide getriggert nach `started`; überlappt nicht wenn Overlay bereits aktiv.
- Verifiziert: Backend-Roundtrip (alle neuen Felder gespeichert + öffentlich lesbar), Screenshots (Code-Warnung bei `<script>alert(1)`, Werbung-Tab mit Wiederholung+Post-Roll, Verifizierungs-Tab mit 3 Feldern). Recurring/Post-Roll-Timing ist minutenbasiert (min 1 Min), daher nicht live durchgetestet — Overlay-Rendering entspricht dem funktionierenden Pre-Roll.
- ERFORDERT REDEPLOY für Production.

## Werbe-Limit + Verify-Test + Werbe-Vorschau (2026-06)
- **Wiederholungs-Limit (Task 1)**: Neues Setting `ad_preroll_repeat_max` (0=unbegrenzt; SettingsInput, DEFAULT_SETTINGS, PUBLIC_SETTINGS_KEYS, SettingsContext FALLBACK). Admin-Feld `setting-ad_preroll_repeat_max` unter Wiederholungs-Einstellungen. EmbedPlayer übergibt `repeatMax`; VideoPlayer zählt via `repeatCountRef`, stoppt setInterval bei Erreichen des Limits.
- **Verify-Test-Buttons (Task 2)**: Pro Dienst Button `verify-test-{key}` (`admin.verify.test`) im Verify-Tab. `testVerify(key)` prüft `document.head` auf das Meta-Tag (META_NAME-Mapping) und vergleicht content mit dem Feld-Token (extractToken akzeptiert Token ODER volles Meta). Ergebnis-Status: ok/missing/mismatch/empty mit farbiger Meldung (`verify-result-{key}`). Prüft den ausgelieferten DOM-Zustand (nach Speichern/Injektion).
